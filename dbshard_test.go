...
func BenchmarkRoute2DB2(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if !rc.DBNode.IsMaster {
			...
			b.Errorf("dbname: %s, tablename: %d, should be master.", rc.DBNode.Ip, rc.DBNode.Port)
		}
		if !rc.DBNode.DBEnable {
...
func BenchmarkRoute2DBs1(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if rc == nil || len(rc) <= 0 {
			...
			b.Errorf("%s, %s, result empty", "db", "table")
		}
		...
		for i, v := range rc {
			...
			if v == nil || v.DBNode == nil {
				...
				b.Errorf("%s, %s, idx: %d empty", "db", "talbe", i)
			}
			if v.DBNode.IsMaster {

				b.Errorf("%s, %d should not be master", v.DBNode.Ip, v.DBNode.Port)
			}
			if !v.DBNode.DBEnable {

				b.Errorf("%s, %d disable", v.DBNode.Ip, v.DBNode.Port)
			}
...
func BenchmarkRoute2DBs2(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for i, v := range rc {
			...
			if !v.DBNode.IsMaster {
				...
				b.Errorf("%s, %d should be master", v.DBNode.Ip, v.DBNode.Port)
			}
			if !v.DBNode.DBEnable {
